...
const PostDetails = (ctx) => {
  ...
  const handleLike = async () => {
    try {
      const res = await fetch(
        `https://localhost:3000/api/post/${ctx.params.id}/like`,
        {
          headers: {
            Authorization: `Bearer ${session?.user?.accessToken}`,
          },
        ...
        <div className="container px-5 py-12 mx-auto ">
          <div className="flex flex-wrap -m-12">
            <div className="p-12 flex flex-col">
              ...
              <span className="text-center py-2 px-2 rounded bg-indigo-50  text-indigo-500 text-xs font-medium tracking-widest">
                {postDetails.category}
              </span>
              <p className="leading-relaxed mb-8">{postDetails.desc}</p>
